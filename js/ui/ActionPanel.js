...
export class ActionPanel {
  ...
  render(state, currentTab) {
    ...
    // Render based on tab
    this._renderDailyActionStatus(state);
    ...
    switch (currentTab) {
      case "match":
        ...
        if (
          this.inMatch &&
          this.matchView.simulator?.matchState &&
          !this.matchView.simulator.matchState.finished
        ) {
  ...
  _advanceToNextShow(player) {
    ...
    const step = () => {
      ...
      const rosterIds = (promotion.roster || []).filter(
        (id) => id !== player.id,
      );
      let opponent = null;
  ...
  renderMatchTab(state) {
    ...
    if (contract && contract.promotionId) {
      ...
      const advanceCard = this.createActionCard(
        ...
        () => {
          const pendingActions = WorldSimulator.tick(
          ...
          // Events have a 30% chance to occur (average every 3-4 days)
          if (Math.random() < 0.3) {
            const event = eventManager.generateEvents(
              player,
              gameStateManager.getStateRef(),
            );
            if (event) {
  ...
  renderPromoOptions(player) {
    ...
    tones.forEach((tone) => {
      const card = this.createActionCard(tone.name, tone.description, () => {
        ...
        if (result.error) {
          ...
          if (result.momentumGained > 0)
            resultText += ` Momentum +${result.momentumGained}.`;
          if (result.momentumGained < 0)
            resultText += ` Momentum ${result.momentumGained}.`;
          if (result.overnessGained > 0)
            resultText += ` Overness +${result.overnessGained}.`;
          if (result.overnessGained < 0)
            resultText += ` Overness ${result.overnessGained}.`;
          if (result.charismaGained > 0)
            resultText += ` Charisma +${result.charismaGained.toFixed(1)}.`;
  ...
  renderBackstageTab(state) {
    ...
    const actions = [
      {
        name: "Visit Booker's Office",
        desc: "Discuss your position and request matches",
        icon: "📋",
        action: "booker",
      },
      {
        name: "Hang out in Locker Room",
        desc: "Build relationships with other wrestlers",
        icon: "🗣️",
        action: "locker",
      },
      {
        name: "Check Dirt Sheets",
        desc: "Read the latest wrestling news and rumors",
        icon: "📰",
        action: "dirt",
      },
      {
        name: "View Relationships",
        desc: "See who are your allies and rivals",
        icon: "👥",
  ...
  renderBookerOffice(player) {
    ...
    const positionInfo = CardPositionSystem.getPositionInfo(
    ...
    const titleShotCard = this.createActionCard(
      ...
      () => {
        ...
        const challengableTitles = championships.filter(
          (c) => c && c.currentChampionId !== player.id,
        );
        ...
        if (
          ...
          msg =
            "The booker agrees! You'll get a title shot at the next big show.";
          if (contract) {
    ...
    const positionCard = this.createActionCard(
      ...
      () => {
        ...
        if (
          ...
          const currentIdx = positions.indexOf(
            contract.position || "dark_match",
          );
          if (currentIdx < positions.length - 1) {
            contract.position = positions[currentIdx + 1];
            const newPosition = CardPositionSystem.getPositionInfo(
              contract.position,
            ).name;
            gameStateManager.dispatch("ADD_LOG_ENTRY", {
              entry: {
                category: "backstage",
                text: `📋 The booker gives you a push! New position: ${newPosition}`,
                type: "booker",
              },
            });
          } else {
            gameStateManager.dispatch("ADD_LOG_ENTRY", {
              entry: {
                category: "backstage",
                text: "📋 You're already at the top of the card!",
                type: "booker",
              },
            });
          }
          ...
          gameStateManager.dispatch("ADD_LOG_ENTRY", {
            entry: {
              category: "backstage",
              text: "📋 The booker doesn't think you're ready for a push yet.",
              type: "booker",
            },
          });
        }
      ...
      () => {
        ...
        if (result.outcome === "CRITICAL_SUCCESS") {
          gameStateManager.dispatch("ADD_LOG_ENTRY", {
            entry: {
              category: "backstage",
              text: "📋 Your passionate speech impressed the booker. They'll consider your feedback.",
              type: "booker",
            },
          });
          if (popularity)
            popularity.momentum = Math.min(100, popularity.momentum + 10);
        } else if (result.outcome === "CRITICAL_FAILURE") {
          gameStateManager.dispatch("ADD_LOG_ENTRY", {
            entry: {
              category: "backstage",
              text: "📋 The booker is furious! You've been buried. Expect a demotion.",
              type: "booker",
            },
          });
          const positions = [
          ...
          if (popularity)
            popularity.momentum = Math.max(0, popularity.momentum - 15);
        } else {
          gameStateManager.dispatch("ADD_LOG_ENTRY", {
            entry: {
              category: "backstage",
              text: "📋 The booker listened but nothing will change.",
              type: "booker",
            },
          });
        }
  ...
  renderLockerRoom(player) {
      ...
      () => {
        rosterMembers.slice(0, 4).forEach((member) => {
          RelationshipManager.modifyAffinity(
            ...
            member.id,
            2,
            "Playing cards in locker room",
          );
        });
        gameStateManager.dispatch("ADD_LOG_ENTRY", {
          entry: {
            category: "backstage",
            text: "🃏 You hung out and played cards. Everyone seems friendlier.",
            type: "social",
          },
        });
        this.renderLockerRoom(player);
    ...
    const quietCard = this.createActionCard(
      ...
      () => {
        const condition = player.getComponent("condition");
        if (condition)
          condition.mentalHealth = Math.min(
            100,
            (condition.mentalHealth || 75) + 5,
          );
        gameStateManager.dispatch("ADD_LOG_ENTRY", {
          entry: {
            category: "backstage",
            text: "🧘 You kept to yourself and focused on your mental game.",
            type: "social",
          },
        });
        this.render(gameStateManager.getStateRef(), "backstage");
      ...
      const card = this.createActionCard(
        ...
        () => {
          ...
          gameStateManager.dispatch("ADD_LOG_ENTRY", {
            entry: {
              category: "backstage",
              text: `🗣️ You spent time with ${identity?.name}. Relationship +${change}.`,
              type: "social",
            },
          });
          this.renderLockerRoom(player);
  ...
  renderRelationshipActions(player, target, relationship) {
    ...
    const socializeCard = this.createActionCard(
      ...
      () => {
        ...
        this.renderRelationshipActions(
          player,
          target,
          this._getRel(player.id, target.id),
        );
      },
    );
    ...
    if (playerAge >= 18 && targetAge >= 18) {
      ...
      if (rel && relTags.includes("romantic")) {
        const breakupCard = this.createActionCard(
          "Break Up",
          rel.committed
            ? "End your committed relationship"
            : "End your romantic connection",
          () => {
  ...
  renderDirtSheets() {
    ...
    if (stories.length === 0) {
      gameStateManager.dispatch("ADD_LOG_ENTRY", {
        entry: {
          category: "system",
          text: "📰 You check the dirt sheets... No major news this week.",
          type: "dirtsheet",
        },
      });
    } else {
  ...
  renderActionsTab(state) {
    ...
    const card = this.createActionCard(
      "Manage Finances",
      "Check your bank balance and expenses",
      () => {
  ...
  renderWellnessOptions(player) {
    ...
    const pedCard = this.createActionCard(
      ...
      wellness?.pedUsage
        ...
        : "Boost stats but risk detection",
      () => {
  ...
  renderLifestyleOptions(player) {
    ...
    // Vacation option
    const vacationCard = this.createActionCard(
      "Take Vacation",
      "1 week, -40 burnout, -$2000",
      () => {
    ...
    // Therapy option
    const therapyCard = this.createActionCard(
      "See Therapist",
      "Improve mental health, -$100",
      () => {
   ...
   */
  createActionCard(title, description, onClick, options = {}) {
    return this.createActionCardWithOptions(
      title,
      description,
      onClick,
      options,
  ...
  createActionCardWithOptions(title, description, onClick, options = {}) {
    const { costsAction = true, actionCost = 1 } = options;
    const card = document.createElement("div");
    ...
    card.addEventListener("click", () => {
      if (costsAction && !this._consumeActionPoints(actionCost, title)) {
        return;
      }
      onClick();
    });
  ...
  _getActionBudgetState(state) {
    if (!state.eventFlags) state.eventFlags = {};
    const calendar = state.calendar || {};
    const dayKey = `${calendar.year || 1}-${calendar.month || 1}-${calendar.week || 1}-${calendar.day || 0}`;

    if (state.eventFlags.__actionBudgetDayKey !== dayKey) {
      state.eventFlags.__actionBudgetDayKey = dayKey;
      state.eventFlags.__actionsUsedToday = 0;
    }

    return {
      dayKey,
      used: state.eventFlags.__actionsUsedToday || 0,
      limit:
        state.eventFlags.__actionsDailyLimit || this.defaultDailyActionLimit,
    };
  }

  _consumeActionPoints(cost = 1, actionTitle = "Action") {
    const state = gameStateManager.getStateRef();
    if (!state) return true;
    const budget = this._getActionBudgetState(state);
    const remaining = budget.limit - budget.used;

    if (remaining < cost) {
      alert(
        `No action points left today. Advance time to take more actions.\nUsed: ${budget.used}/${budget.limit}`,
      );
      return false;
    }

    state.eventFlags.__actionsUsedToday = budget.used + cost;
    gameStateManager.dispatch("ADD_LOG_ENTRY", {
      entry: {
        category: "system",
        text: `${actionTitle} used ${cost} action point${cost > 1 ? "s" : ""}. (${state.eventFlags.__actionsUsedToday}/${budget.limit} used today)`,
        type: "action",
      },
    });
    return true;
  ...
  _renderDailyActionStatus(state) {
    const player = gameStateManager.getPlayerEntity();
  ...
  resolveEventChoice(event, choiceIndex) {
    ...
    const result = eventManager.resolveChoice(
      event,
      choiceIndex,
      player,
      state,
    ...
    if (
      event.id === "scout_notice" &&
      result.generatedOffer &&
      result.generatedPromotionId
    ) {
  ...
  renderEventOutcome(event, result) {
    ...
    if (result.resolutionResult) {
      ...
      let resText = "Success";
      if (result.resolutionResult.outcome === "CRITICAL_SUCCESS")
        resText = "Critical Success";
      if (result.resolutionResult.outcome === "FAILURE") resText = "Failure";
      if (result.resolutionResult.outcome === "CRITICAL_FAILURE")
        resText = "Critical Failure";
      ...
      const chance = result.resolutionResult.successRate;
      const difficultyPenalty = result.resolutionResult.difficultyPenalty || 0;

      let rollText = `Chance: ${chance.toFixed(1)}% | Roll: ${total.toFixed(1)} (${resText}) | DC ${dc}`;
      if (modifier !== 0 || difficultyPenalty !== 0) {
        const modSign = modifier >= 0 ? "+" : "";
        rollText += ` | Mod ${modSign}${modifier.toFixed(1)} | DC Penalty ${difficultyPenalty.toFixed(1)}`;
      }
  ...
  _renderTrainingCards(player, returnTab) {
    ...
    categories.forEach((cat) => {
      const displayRisk = summary.overtraining
        ? cat.injuryRisk * 2
        : cat.injuryRisk;
  ...
  renderMatchPreparation(match) {
    ...
    const opponentName =
      match.opponent?.getComponent("identity")?.name || "Unknown";
    const bookedText =
      match.bookedWinner === "wrestler1"
    ...
    const startBtn = this.createActionCard(
      "Start Match",
      "Enter the ring",
      () => {
  ...
  startMatch(match) {
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        const preMatchTitle =
          match.isTitleMatch && match.titleId
            ? stateRef.championships.get(match.titleId)
            : null;
        const wasPlayerChampionBefore =
          preMatchTitle?.currentChampion === match.player.id;

        // Determine winner/loser entities
        const playerWon = winnerEntityId
          ? winnerEntityId === match.player.id
          : winnerSide === "wrestler1";
  ...
  renderWrestlerDetails(entity) {
    ...
    if (!samePromotion) {
      ...
      info.innerHTML =
        "<p>You can only book matches/feuds with wrestlers in your current promotion.</p>";
      this.container.appendChild(info);
    }
    ...
    if (samePromotion) {
      ...
      const feudCard = this.createActionCard(feudLabel, feudDesc, () =>
  ...
  askForMatch(player, opponent) {
    ...
    const feudMatchTypes = activeFeud
      ? DynamicFeudSystem.getFeudMatchTypes(activeFeud.id)
      : [];
    ...
    const matchType = feudMatchTypes.length
      ? feudMatchTypes[0]
      : "Standard Singles";
  ...
  handleFeudAction(player, opponent) {
    ...
    const startResult = DynamicFeudSystem.startFeud(
      player,
      opponent,
      "Challenge issued in the locker room",
    );
  ...
  talkTrash(player, target) {
    ...
    if (!this._getActiveFeud(player.id, target.id) && Math.random() < 0.25) {
      DynamicFeudSystem.startFeud(
        player,
        target,
        "Backstage trash talk crossed the line",
      );
    }
  ...
  flirtWithWrestler(player, target) {
    ...
    RelationshipManager.setRelationship(player.id, target.id, {
      ...
      trust: Math.max(
        0,
        Math.min(100, (rel.trust ?? 50) + (delta > 0 ? 4 : -5)),
      ),
    });
    ...
    gameStateManager.dispatch("ADD_LOG_ENTRY", {
      entry: {
        ...
        text:
          delta > 0
            ? `💘 Your flirting lands well with ${targetName}.`
            : `💔 ${targetName} is not impressed by your flirting.`,
        type: "social",
      },
    });
    this.renderWrestlerDetails(target);
  ...
  askOnDate(player, target) {
    ...
    gameStateManager.dispatch("ADD_LOG_ENTRY", {
      entry: {
        category: "backstage",
        text: success
          ? `💘 ${targetName} agrees to a date. Romance is building.`
          : `💔 ${targetName} declines the date invitation.`,
        type: "social",
      },
    });
    this.renderWrestlerDetails(target);
  ...
  intimateEncounter(player, target) {
    ...
    const narrative =
      dataManager
        .getRandomNarrative(narrativeKey)
        ?.replace("{actor}", "You")
        .replace("{target}", targetName) ||
      `You and ${targetName} share an intimate moment.`;
  ...
  explicitSecretAffair(player, target) {
    ...
    if (success) {
      ...
      RelationshipManager.modifyAffinity(
        ...
        target.id,
        12,
        "Explicit affair began",
      );
      narrative =
        dataManager
          .getRandomNarrative("nsfw_affair_success")
          ?.replace("{actor}", "You")
          .replace("{target}", targetName) ||
        `You and ${targetName} share a passionate but risky intimate encounter.`;
    } else {
      RelationshipManager.modifyAffinity(
        player.id,
        target.id,
        -15,
        "Explicit affair attempt failed",
      );
      const narrativeKey =
        result.outcome === "CRITICAL_FAILURE"
          ? "nsfw_intimate_critical_failure"
          : "nsfw_intimate_failure";
      narrative =
        dataManager
          .getRandomNarrative(narrativeKey)
          ?.replace("{actor}", "You")
          .replace("{target}", targetName) ||
        `The secret approach fails and creates tension with ${targetName}.`;
    }
  ...
  groupEncounter(player, target) {
    ...
    const sortedRelationships = relationships
      ...
      .filter((rel) => {
        if (!rel.entity || rel.id === player.id || rel.id === target.id)
          return false;
        ...
        if (promotion?.roster) {
          const promotionPartners = promotion.roster
            ...
            .filter((p) => {
              if (
                !p.entity ||
                p.entity.id === player.id ||
                p.entity.id === target.id
              )
                return false;
    ...
    const targetName = target.getComponent("identity")?.name || "them";
    const partnerNames = potentialPartners
      .map((p) => p.getComponent("identity")?.name || "someone")
      .join(", ");
    let narrative;
    let romanceDelta;
    let trustDelta;
    ...
    if (exposure) {
      ...
      gameStateManager.dispatch("ADD_LOG_ENTRY", {
        entry: {
          category: "backstage",
          text: success
            ? "🕶️ A secret affair begins quietly."
            : "💥 The secret approach fails and creates tension.",
          type: "social",
        },
      });
    }
  ...
  renderCareerTab(state) {
    ...
    if (contract?.promotionId) {
      ...
      if ((contract.renewalWindowWeeks || 0) > 0) {
        ...
        const renewalOfferCard = this.createActionCard(
          ...
          () => {
            const renewalOffer =
              contract.pendingRenewalOffer ||
              ContractEngine.generateRenewalOffer(player);
            if (renewalOffer) {
              ...
              gameStateManager.dispatch("ADD_LOG_ENTRY", {
                entry: {
                  category: "personal",
                  text: "No renewal offer available right now.",
                  type: "contract",
                },
              });
          ...
          () => {
            ...
            gameStateManager.dispatch("ADD_LOG_ENTRY", {
              entry: {
                category: "personal",
                text: result?.success
                  ? "You declined renewal and entered free agency."
                  : result?.error || "Could not decline renewal.",
                type: "contract",
              },
            });
            this.render(gameStateManager.getStateRef(), "career");
    ...
    const browseCard = this.createActionCard(
      ...
      contract?.promotionId
        ? "View other promotions and contract offers"
        : "Find a promotion to sign with",
      () => {
    ...
    // Perks
    const perkCard = this.createActionCard(
      "View Perks",
      "See unlocked and active perks",
      () => {
  ...
  renderPromotionBrowser(player) {
    ...
    const promotions = Array.from(state.promotions.values()).sort(
    ...
    promotions.forEach((promo) => {
      ...
      const canApply =
        !isCurrent &&
        playerOverness >= minOverness &&
        !currentContract?.promotionId;
  ...
  renderContractOffer(player, promotion, existingOffer = null) {
    ...
    const baseOffer =
      existingOffer || ContractEngine.generateOffer(promotion, player);
    const offer = {
      ...baseOffer,
      ...
      remainingWeeks: Math.max(
        1,
        baseOffer?.remainingWeeks ?? baseOffer?.lengthWeeks ?? 8,
      ),
      ...
      titleOpportunityGuaranteed:
        baseOffer?.titleOpportunityGuaranteed === true,
      championshipOpportunityWeeks:
        baseOffer?.championshipOpportunityWeeks ?? 0,
    ...
    this.careerView = "offer";
    const positionInfo = CardPositionSystem.getPositionInfo(
      this.currentOffer.position || "mid_card",
    );
    ...
    const negotiateCard = this.createActionCard(
      ...
      () => {
        const result = ContractEngine.negotiateClause(
          player,
          "weeklySalary",
          this.currentOffer,
          Math.floor(this.currentOffer.weeklySalary * 1.5),
        );
        ...
        gameStateManager.dispatch("ADD_LOG_ENTRY", {
          entry: {
            category: "personal",
            text: `💰 ${result.narrative} Salary: $${result.resultValue}/week`,
            type: "negotiation",
          },
        });
        this.renderContractOffer(player, promotion, this.currentOffer);
    ...
    const negotiateLengthCard = this.createActionCard(
      ...
      () => {
        ...
        gameStateManager.dispatch("ADD_LOG_ENTRY", {
          entry: {
            category: "personal",
            text: `📅 ${result.narrative} Length: ${result.resultValue} weeks`,
            type: "negotiation",
          },
        });
        this.renderContractOffer(player, promotion, this.currentOffer);
    ...
    const negotiateMerchCard = this.createActionCard(
      ...
      () => {
        ...
        gameStateManager.dispatch("ADD_LOG_ENTRY", {
          entry: {
            category: "personal",
            text: `🛍️ ${result.narrative} Merch Cut: ${result.resultValue}%`,
            type: "negotiation",
          },
        });
        this.renderContractOffer(player, promotion, this.currentOffer);
    ...
    const negotiateCreativeCard = this.createActionCard(
      ...
      () => {
        ...
        gameStateManager.dispatch("ADD_LOG_ENTRY", {
          entry: {
            category: "personal",
            text: `🎭 ${result.narrative} Creative Control: ${this.currentOffer.hasCreativeControl ? "Yes" : "No"}`,
            type: "negotiation",
          },
        });
        this.renderContractOffer(player, promotion, this.currentOffer);
    ...
    const negotiateInjuryCard = this.createActionCard(
      ...
      () => {
        const target = Math.min(
          100,
          (this.currentOffer.injuryCoveragePct || 0) + 20,
        );
        const result = ContractEngine.negotiateClause(
          player,
          "injuryCoveragePct",
        ...
        gameStateManager.dispatch("ADD_LOG_ENTRY", {
          entry: {
            category: "personal",
            text: `🩺 ${result.narrative} Injury Coverage: ${result.resultValue}%`,
            type: "negotiation",
          },
        });
        this.renderContractOffer(player, promotion, this.currentOffer);
    ...
    const negotiateDatesCard = this.createActionCard(
      ...
      () => {
        ...
        gameStateManager.dispatch("ADD_LOG_ENTRY", {
          entry: {
            category: "personal",
            text: `🗓️ ${result.narrative} Dates/Month: ${result.resultValue}`,
            type: "negotiation",
          },
        });
        this.renderContractOffer(player, promotion, this.currentOffer);
    ...
    const negotiateTitleRunCard = this.createActionCard(
      ...
      () => {
        const clauseResult = ContractEngine.negotiateClause(
          player,
          "titleOpportunityGuaranteed",
          this.currentOffer,
          true,
        ...
        if (this.currentOffer.titleOpportunityGuaranteed) {
          const weeksResult = ContractEngine.negotiateClause(
            player,
            "championshipOpportunityWeeks",
            this.currentOffer,
            12,
          );
          this.currentOffer.championshipOpportunityWeeks =
            weeksResult.resultValue;
          gameStateManager.dispatch("ADD_LOG_ENTRY", {
            entry: {
              category: "personal",
              text: `🏆 ${weeksResult.narrative} Title opportunity within ${weeksResult.resultValue} weeks.`,
              type: "negotiation",
            },
          });
        } else {
    ...
    const declineCard = this.createActionCard(
      ...
      () => {
        gameStateManager.dispatch("ADD_LOG_ENTRY", {
          entry: {
            category: "personal",
            text: `Declined contract offer from ${promotion.name}.`,
            type: "contract",
          },
        });
  ...
  renderPerkView(player) {
    ...
    const unlockedInactive = perkData.unlocked.filter(
      (perk) => !activePerkIds.has(perk.id),
    );
