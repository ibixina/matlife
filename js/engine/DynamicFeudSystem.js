...
const FEUD_PHASES = {
  tension: {
    ...
    escalationEvents: [
      {
        type: "confrontation",
        description: "A heated verbal exchange in the locker room",
        heatGain: 15,
      },
      {
        type: "disrespect",
        description: "One wrestler refused to shake hands",
        heatGain: 10,
      },
      {
        type: "mockery",
        description: "Mocking interview on local TV",
        heatGain: 12,
      },
      {
        type: "sneer",
        description: "Intimidating staredown during entrance",
        heatGain: 8,
      },
    ],
  },
  heat: {
    ...
    escalationEvents: [
      {
        type: "brawl",
        description: "A violent backstage brawl left both wrestlers bloodied",
        heatGain: 25,
      },
      {
        type: "ambush",
        description: "Cowardly attack after the match with a steel chair",
        heatGain: 30,
      },
      {
        type: "mockery",
        description: "Despicable promo bringing up family members",
        heatGain: 35,
      },
      {
        type: "destruction",
        description: "Trashed the opponent locker room and gear",
        heatGain: 28,
      },
      {
        type: "interference",
        description: "Cost them the title match with a vicious beatdown",
        heatGain: 32,
      },
    ],
  },
  blowoff: {
    ...
    escalationEvents: [
      {
        type: "threat",
        description: "Vicious threat to end their career",
        heatGain: 40,
      },
      {
        type: "injury",
        description: "Brutal attack sent them to the hospital",
        heatGain: 50,
      },
      {
        type: "desecration",
        description: "Burned their gear and spit on their legacy",
        heatGain: 45,
      },
      {
        type: "ultimatum",
        description: "This ends TONIGHT - one of us is leaving in an ambulance",
        heatGain: 55,
      },
    ],
  },
  bloodfeud: {
    ...
    escalationEvents: [
      {
        type: "bloodshed",
        description: "The hatred is personal. Blood has been spilled.",
        heatGain: 100,
      },
      {
        type: "vengeance",
        description:
          "This isn't about wrestling anymore. It's about destruction.",
        heatGain: 100,
      },
      {
        type: "endgame",
        description: "Only one will survive this war.",
        heatGain: 100,
      },
    ],
  },
...
export class DynamicFeudSystem {
  static _getMatchHeatGain(matchRating, phase) {
    const baseGain = matchRating >= 4.5 ? 16 : matchRating >= 3.5 ? 12 : 8;
      ...
      phase === "tension"
        ...
        : phase === "heat"
          ...
          : phase === "blowoff"
            ? 0.8
            : 0.7;
    return Math.max(5, Math.round(baseGain * phaseMultiplier));
  }

  static _getActivityHeatGain(baseHeatGain) {
    // Keep activity beats meaningful but stop weekly spikes.
    return Math.max(4, Math.round(baseHeatGain * 0.35));
  }
  ...
  static escalateFeud(feudId) {
    ...
    feud.heat = Math.min(100, feud.heat + 10);
  ...
  static escalateAfterMatch(feudId, matchRating, matchResult = null) {
    ...
    const heatGain = this._getMatchHeatGain(matchRating, feud.phase);
    ...
    const phaseKeys = Object.keys(FEUD_PHASES);
    const currentIndex = phaseKeys.indexOf(feud.phase);
    const matchesInPhase = feud.matches.filter(
      (m) => m.week >= (feud.phaseStartWeek || feud.startWeek),
    ).length;

    // Slower phase progression: require sustained build and meaningful heat.
    if (
      currentIndex < phaseKeys.length - 1 &&
      ((matchesInPhase >= 3 && feud.heat >= 45) || matchRating >= 4.8)
    ) {
      return this.escalateFeud(feudId);
    }

    // Resolve winner from actual match result when available.
    const winnerId =
      matchResult?.winnerId || matchResult?.winnerEntityId || entityA?.id;
    const resolvedWinner = state.entities.get(winnerId) || entityA;
    const canResolveOnThisMatch = matchResult?.isBigShow === true;

    // Check if feud should END (blowoff or bloodfeud with enough matches)
    const totalMatches = feud.matches.length;
    if (
      canResolveOnThisMatch &&
      (feud.phase === "blowoff" || feud.phase === "bloodfeud") &&
      totalMatches >= 3
    ) {
      return this.resolveFeud(feudId, resolvedWinner, matchRating, {
        isBigShow: true,
      });
    }
    ...
    if (
      canResolveOnThisMatch &&
      matchResult &&
      (matchResult.matchType === "Retirement Match" ||
        matchResult.matchType === "Loser Leaves Town")
    ) {
      const result = this.resolveFeud(feudId, resolvedWinner, matchRating, {
        isBigShow: true,
      });
   ...
   */
  static resolveFeud(feudId, winner, matchRating, options = {}) {
    const state = gameStateManager.getStateRef();
    const feud = state.feuds.get(feudId);
    ...
    if (options.isBigShow !== true) {
      return { error: "Feud can only be resolved on a big show" };
    }
  ...
  static tickAllFeuds() {
    ...
    for (const [feudId, feud] of state.feuds) {
      ...
      const phaseStartWeek =
        feud.phaseStartWeek || feud.startWeek || state.calendar.absoluteWeek;
      const weeksInPhase = state.calendar.absoluteWeek - phaseStartWeek;
      ...
      if (feud.phase === "heat" && Math.random() < 0.4) {
        ...
        feud.heat = Math.min(
          100,
          feud.heat + this._getActivityHeatGain(event.heatGenerated),
        );
  ...
  static getEntityFeuds(entityId) {
    ...
    for (const [feudId, feud] of state.feuds) {
      if (feud.entityA === entityId || feud.entityB === entityId) {
        const opponentId =
          feud.entityA === entityId ? feud.entityB : feud.entityA;
