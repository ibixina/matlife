...
async function runComprehensiveTests() {
  ...
  try {
    ...
    assert(
      ...
      "Player entity correctly set in state",
    );
    assert(
      player.getComponent("inRingStats").technical >= 21,
      "Bonus points correctly applied to technical stat",
    );
    ...
    assert(
      state.championships.size > 0,
      "Championships initialized across promotions",
    );

    // Give player a contract to the top promotion
    const topPromo = Array.from(state.promotions.values()).sort(
      (a, b) => b.prestige - a.prestige,
    )[0];
    const contractResult = ContractEngine.signContract(
      player,
      topPromo,
      "midcard",
      52,
    ...
    assert(
      player.getComponent("contract").promotionId === topPromo.id,
      "Player contract reflects correct promotion",
    );
    assert(
      topPromo.roster.includes(player.id),
      "Player added to promotion roster",
    );
    ...
    const eligibility = ChampionshipSystem.checkTitleShotEligibility(
      player,
      topPromo,
    );
    assert(
      eligibility.eligible === true,
      "Player eligible for title shot with max stats",
    );
    ...
    assert(
      matchCard.isTitleMatch === true,
      "Show card generator recognized pending title shot and booked title match",
    );
    assert(
      matchCard.titleId !== null,
      "Match card has valid title ID attached",
    );
    assert(
      player.getComponent("contract").pendingTitleShot === false,
      "Pending title shot flag consumed",
    );
    ...
    assert(
      simState.phase === "Feeling Out",
      "Match starts in Feeling Out phase",
    );
    ...
    assert(
      sim.matchState.wrestler1.stamina < 100,
      "Stamina consumed during move",
    );
    ...
    assert(
      sim.getCurrentPhase().name === "The Finish",
      "Correctly transitioned phase based on turns",
    );
    ...
    assert(
      sim.matchState.winner === "wrestler1",
      "Player wins the simulated match",
    );
    ...
    const updatedTitleInfo = ChampionshipSystem.getChampionshipInfo(
      matchCard.titleId,
    );
    assert(
      updatedTitleInfo.currentChampion.name ===
        player.getComponent("identity").name,
      "Title successfully changed hands after win",
    );
    ...
    assert(
      typeof relationship.affinity === "number",
      "Affinity has a valid number",
    );
    ...
    assert(
      ...
      "Health reduced due to injury application",
    );

    InjuryEngine.processDailyRecovery(player);
    assert(
      playerCondition.injuries[0].daysRemaining === 1,
      "Recovery correctly subtracted a day",
    );
    ...
    assert(
      testPlayer.getComponent("contract").remainingWeeks === 1,
      "Contract initialized with 1 week remaining",
    );
    assert(
      testPromotion.roster.includes(testPlayer.id),
      "Player added to promotion roster",
    );
    ...
    const expired =
      playerContract.promotionId === null &&
      playerContract.remainingWeeks === 0;
    assert(
      expired,
      "Contract expired: promotionId cleared and remainingWeeks is 0",
    );
    assert(
      !testPromotion.roster.includes(testPlayer.id),
      "Player removed from promotion roster after contract expiration",
    );

    // Verify player is now a free agent
    assert(
      playerContract.promotionId === null,
      "Player is now a free agent (no promotionId)",
    );
    assert(
      playerContract.weeklySalary === 0,
      "Weekly salary reset to 0 after expiration",
    );
    ...
    try {
      ...
      assert(
        true,
        "Time advancement works correctly after contract expiration",
      );
    } catch (e) {
      assert(
        false,
        `Time advancement crashed after contract expiration: ${e.message}`,
      );
    }

    // Test immediate re-signing after expiration
    const reSignResult = ContractEngine.signContract(
      testPlayer,
      testPromotion,
      "midcard",
      4,
    ...
    assert(
      ...
      "Player can re-sign with promotion after expiration",
    );
    assert(
      testPlayer.getComponent("contract").remainingWeeks === 4,
      "New contract has 4 weeks remaining",
    );
    assert(
      testPromotion.roster.includes(testPlayer.id),
      "Player re-added to promotion roster",
    );
    ...
    assert(
      ncContract.noCompeteWeeksRemaining === 4,
      "No-compete has 4 weeks remaining",
    );

    // Try to sign new contract while no-compete is active
    const blockedSignResult = ContractEngine.signContract(
      noCompetePlayer,
      ncPromotion,
      "midcard",
      4,
    ...
    assert(
      blockedSignResult.success === false,
      "Cannot sign contract while no-compete is active",
    );
    assert(
      blockedSignResult.error.includes("No-compete"),
      "Error message mentions no-compete clause",
    );
    ...
    for (let i = 0; i < 28; i++) {
      ...
      if (
        !ncContract.noCompeteActive ||
        ncContract.noCompeteWeeksRemaining <= 0
      ) {
        break;
      }
    }
    ...
    assert(
      ncContract.noCompeteActive === false ||
        ncContract.noCompeteWeeksRemaining <= 0,
      "No-compete clause expired after daily processing",
    );

    // Now signing should work
    const afterNoCompeteSignResult = ContractEngine.signContract(
      noCompetePlayer,
      ncPromotion,
      "midcard",
      4,
    ...
    assert(
      afterNoCompeteSignResult.success,
      "Can sign contract after no-compete expires",
    );
    ...
    assert(
      burnoutLifestyle.burnout < 80,
      `Burnout decreased from 80 to ${burnoutLifestyle.burnout} due to natural decay`,
    );
    ...
    const injuredPlayerCondition = injuredPlayer.getComponent("condition");
    const seriousInjuries = injuredPlayerCondition.injuries.filter(
      (i) => i.severity >= 3,
    );
    ...
    console.log(
      `  Event generation ${generatedEvent ? "successful" : "returned null"} (may be null based on conditions)`,
    );

    // ---------------------------------------------------------
    // 12. Booker Mode Core Loop
    // ---------------------------------------------------------
    console.log("\n--- 12. Booker Mode Core Loop ---");

    const bookerPlayer = EntityFactory.createPlayerBooker({
      bookerName: "Booker Test",
      age: 34,
      promotionName: "Booked To Win",
      promotionRegion: "USA",
      bookingStyle: "storyteller",
      productStyle: "Mixed",
      sandboxMode: false,
    });
    bookerPlayer.isPlayer = true;

    gameStateManager.initializeState({
      playerId: bookerPlayer.id,
      mode: "BOOKER",
    });
    const bookerState = gameStateManager.getStateRef();
    gameStateManager.dispatch("ADD_ENTITY", { entity: bookerPlayer });

    const creator = new CharacterCreation(null);
    AIPromotionSystem.generateInitialPromotions(bookerState, 4);
    creator.generateNPCRosters(bookerState);
    creator.generateFreeAgents(bookerState, 12);

    const playerPromotion = BookerModeEngine.createPlayerPromotion(
      {
    ...
    assert(
      playerPromotion !== null,
      "Booker mode creates a player-owned promotion",
    );
    assert(
      bookerState.player.promotionId === playerPromotion.id,
      "Player promotion ID stored in state",
    );
    assert(
      playerPromotion.roster.length >= 8,
      "Player promotion starts with a usable roster",
    );

    for (const promotion of bookerState.promotions.values()) {
      const hasTitles = Array.from(bookerState.championships.values()).some(
    ...
    const show = BookerModeEngine.autoBookCurrentShow(
      playerPromotion,
      bookerState,
    );
    assert(
      ...
      "Auto-booking fills at least one match slot",
    );

    const showReport = BookerModeEngine.runCurrentShow(
      playerPromotion,
      bookerState,
    );
    assert(
      showReport.results.length > 0,
      "Running a booked show produces segment results",
    );
    assert(
      showReport.showRating >= 1.2 && showReport.showRating <= 5.0,
      "Show rating stays within expected bounds",
    );
    assert(
      playerPromotion.showHistory.length === 1,
      "Show history records completed events",
    );
    assert(
      playerPromotion.creative.currentShow.week >=
        bookerState.calendar.absoluteWeek,
      "A future current show is generated after running the card",
    );

    // Weekly hook should not crash and should keep scouting data fresh
    BookerModeEngine.processWeekly(playerPromotion, bookerState);
    assert(
      playerPromotion.scoutingReport.length > 0,
      "Weekly processing refreshes the scouting report",
    );
  } catch (error) {
